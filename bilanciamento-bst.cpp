...
public:
    ...
    void inorder(Node<H> * aux, int &cont){
        if(aux!=NULL){
    ...
    int bilanciamento(Node<H> * aux){
        if(aux!=NULL){
            int contdx=0, contsx=0;
            inorder(aux->getLeft(),contsx);
            inorder(aux->getRight(),contdx);
            int result=contdx-contsx;
            if(result<0){
                return result * (-1);
            }else{
                return result;
...
int main(){
    ...
    for(int i=0;i<100;i++){
        ...
        if(type=="int"){
            ...
            int dato;
            in>>dato;
            out<<b1->bilanciamento(b1->searchNode(dato))<<endl;
        }else if(type=="double"){
